...
static void send_nav(httpd_req_t *req) {
  httpd_resp_sendstr_chunk(
    ...
    "<a href=\"#controller-section\">Controller</a>"
    "<a href=\"#network-section\">Network</a>"
    "<a href=\"#cloud-section\">Cloud</a>"
...
static void send_temperature_step_options(httpd_req_t *req, float selected_step) {
  httpd_resp_sendstr_chunk(req, "<option value=\"0.1\"");
  if (selected_step < 0.3f) {
    httpd_resp_sendstr_chunk(req, " selected");
  }
  httpd_resp_sendstr_chunk(req, ">0.1 C</option><option value=\"0.5\"");
  if (selected_step >= 0.3f) {
    httpd_resp_sendstr_chunk(req, " selected");
  }
  httpd_resp_sendstr_chunk(req, ">0.5 C</option>");
}

static void send_time_step_options(httpd_req_t *req, float selected_step) {
  httpd_resp_sendstr_chunk(req, "<option value=\"0.1\"");
  if (selected_step < 0.3f) {
    httpd_resp_sendstr_chunk(req, " selected");
  }
  httpd_resp_sendstr_chunk(req, ">0.1 s</option><option value=\"0.5\"");
  if (selected_step >= 0.3f) {
    httpd_resp_sendstr_chunk(req, " selected");
  }
  httpd_resp_sendstr_chunk(req, ">0.5 s</option>");
}

static void render_overview_section(httpd_req_t *req, const lm_ctrl_setup_portal_view_t *view) {
  httpd_resp_sendstr_chunk(req, "<section id=\"overview-section\" class=\"section-card preset-anchor\"><div class=\"section-head\"><h2>Overview</h2><p>Current portal reachability and controller summary.</p></div>");
  httpd_resp_sendstr_chunk(req, "<pre>");
  httpd_resp_sendstr_chunk(req, view->status_html);
  httpd_resp_sendstr_chunk(req, "</pre><p class=\"section-note\">Use <strong>");
  httpd_resp_sendstr_chunk(req, view->local_url_html);
  httpd_resp_sendstr_chunk(req, "</strong> in your home network. The router IP can change after reconnects or flashes.</p></section>");
}
...
static void render_controller_section(httpd_req_t *req, const lm_ctrl_setup_portal_view_t *view) {
  ...
  httpd_resp_sendstr_chunk(req, ">English</option><option value=\"de\"");
...
static void render_cloud_section(httpd_req_t *req, const lm_ctrl_setup_portal_view_t *view) {
  ...
  if ((view->dashboard_feature_mask & LM_CTRL_MACHINE_FEATURE_BBW) != 0) {
    ...
    httpd_resp_sendstr_chunk(req, ">Continuous</option></select><div class=\"field-grid\"><div><label for=\"bbw_dose_1\">Dose 1 Target (g)</label><input id=\"bbw_dose_1\" name=\"bbw_dose_1\" type=\"number\" min=\"5\" max=\"100\" step=\"0.1\" value=\"");
    httpd_resp_sendstr_chunk(req, bbw_dose_1);
    httpd_resp_sendstr_chunk(req, "\"></div><div><label for=\"bbw_dose_2\">Dose 2 Target (g)</label><input id=\"bbw_dose_2\" name=\"bbw_dose_2\" type=\"number\" min=\"5\" max=\"100\" step=\"0.1\" value=\"");
    httpd_resp_sendstr_chunk(req, bbw_dose_2);
    httpd_resp_sendstr_chunk(req, "\"></div></div><div class=\"section-note\">These values are cloud-only and will also be available on the round controller when BBW is supported.</div><button type=\"submit\">Save Brew By Weight Settings</button></form>");
  }

  httpd_resp_sendstr_chunk(req, "</section>");
}

static void render_recipe_card(httpd_req_t *req, const lm_ctrl_setup_portal_view_t *view, int preset_index, bool open_by_default) {
  ...
  char preset_summary[160];
  char preset_summary_html[224];
  ...
  snprintf(
    preset_summary,
    sizeof(preset_summary),
    "Coffee Boiler %.1f C · On-Time %.1f s · Off-Time %.1f s",
    (double)preset->values.temperature_c,
    (double)preset->values.infuse_s,
    (double)preset->values.pause_s
  );
  html_escape_text(preset_summary, preset_summary_html, sizeof(preset_summary_html));

  httpd_resp_sendstr_chunk(req, "<details class=\"preset-card\"");
  if (open_by_default) {
    httpd_resp_sendstr_chunk(req, " open");
  }
  httpd_resp_sendstr_chunk(req, "><summary><div><strong>");
  httpd_resp_sendstr_chunk(req, preset_title);
  httpd_resp_sendstr_chunk(req, "</strong><div class=\"summary-name\">");
  ...
  httpd_resp_sendstr_chunk(req, "\"><div class=\"field-grid\"><div><label>Temperature (C)</label><input name=\"temperature_c\" type=\"number\" min=\"80\" max=\"103\" step=\"");
  httpd_resp_sendstr_chunk(req, view->temperature_step_c < 0.3f ? "0.1" : "0.5");
  httpd_resp_sendstr_chunk(req, "\" value=\"");
  httpd_resp_sendstr_chunk(req, temperature_text);
  httpd_resp_sendstr_chunk(req, "\"></div><div><label>Prebrewing In (s)</label><input name=\"infuse_s\" type=\"number\" min=\"0\" max=\"9\" step=\"");
  httpd_resp_sendstr_chunk(req, view->time_step_s < 0.3f ? "0.1" : "0.5");
  httpd_resp_sendstr_chunk(req, "\" value=\"");
  httpd_resp_sendstr_chunk(req, infuse_text);
  httpd_resp_sendstr_chunk(req, "\"></div><div><label>Prebrewing Out (s)</label><input name=\"pause_s\" type=\"number\" min=\"0\" max=\"9\" step=\"");
  httpd_resp_sendstr_chunk(req, view->time_step_s < 0.3f ? "0.1" : "0.5");
  httpd_resp_sendstr_chunk(req, "\" value=\"");
  ...
  if ((view->dashboard_feature_mask & LM_CTRL_MACHINE_FEATURE_BBW) != 0) {
    httpd_resp_sendstr_chunk(req, "<div class=\"field-grid\"><div><label>BBW Mode</label><select name=\"bbw_mode\"><option value=\"Dose1\"");
    ...
    httpd_resp_sendstr_chunk(req, ">Continuous</option></select></div><div><label>BBW Dose 1 (g)</label><input name=\"bbw_dose_1\" type=\"number\" min=\"5\" max=\"100\" step=\"0.1\" value=\"");
    httpd_resp_sendstr_chunk(req, bbw_dose_1_text);
    httpd_resp_sendstr_chunk(req, "\"></div><div><label>BBW Dose 2 (g)</label><input name=\"bbw_dose_2\" type=\"number\" min=\"5\" max=\"100\" step=\"0.1\" value=\"");
    httpd_resp_sendstr_chunk(req, bbw_dose_2_text);
    httpd_resp_sendstr_chunk(req, "\"></div></div>");
  }

  httpd_resp_sendstr_chunk(req, "<button type=\"submit\">Save Preset</button></form></details>");
}

static void render_recipes_section(httpd_req_t *req, const lm_ctrl_setup_portal_view_t *view) {
  char preset_count_text[8];

  snprintf(preset_count_text, sizeof(preset_count_text), "%u", (unsigned)view->preset_count);
  httpd_resp_sendstr_chunk(req, "<section id=\"recipes-section\" class=\"stack preset-anchor\"><div class=\"section-head\"><h2>Recipes</h2><p>Edit only the active stored preset definitions. Saving here does not immediately send values to the machine.</p></div><div class=\"section-card\"><div class=\"section-note\">Active presets: <strong>");
  httpd_resp_sendstr_chunk(req, preset_count_text);
  httpd_resp_sendstr_chunk(req, "</strong> · Temperature step <strong>");
  httpd_resp_sendstr_chunk(req, view->temperature_step_c < 0.3f ? "0.1 C" : "0.5 C");
  httpd_resp_sendstr_chunk(req, "</strong> · Time step <strong>");
  httpd_resp_sendstr_chunk(req, view->time_step_s < 0.3f ? "0.1 s" : "0.5 s");
  httpd_resp_sendstr_chunk(req, "</strong></div><div class=\"preset-list\">");
  for (int preset_index = 0; preset_index < (int)view->preset_count; ++preset_index) {
    render_recipe_card(req, view, preset_index, preset_index == 0);
  }
  httpd_resp_sendstr_chunk(req, "</div></div></section>");
}

static void render_advanced_section(httpd_req_t *req, const lm_ctrl_setup_portal_view_t *view) {
  char preset_count_text[8];
  ...
  for (int preset_count = 2; preset_count <= CTRL_PRESET_MAX_COUNT; ++preset_count) {
    ...
    snprintf(option_value, sizeof(option_value), "%d", preset_count);
    httpd_resp_sendstr_chunk(req, "<option value=\"");
    httpd_resp_sendstr_chunk(req, option_value);
    httpd_resp_sendstr_chunk(req, "\"");
    if (preset_count == (int)view->preset_count) {
      httpd_resp_sendstr_chunk(req, " selected");
    }
    httpd_resp_sendstr_chunk(req, ">");
    httpd_resp_sendstr_chunk(req, option_value);
  ...
  httpd_resp_sendstr_chunk(req, "</select></div><div><label for=\"temperature_step_c\">Temperature Step</label><select id=\"temperature_step_c\" name=\"temperature_step_c\">");
  send_temperature_step_options(req, view->temperature_step_c);
  httpd_resp_sendstr_chunk(req, "</select></div><div><label for=\"time_step_s\">Time Step</label><select id=\"time_step_s\" name=\"time_step_s\">");
...
static void render_diagnostics_section(httpd_req_t *req, const lm_ctrl_setup_portal_view_t *view) {
  httpd_resp_sendstr_chunk(req, "<section id=\"diagnostics-section\" class=\"stack preset-anchor\"><div class=\"section-head\"><h2>Diagnostics</h2><p>Read-only controller and link state for debugging reconnect or sync issues.</p></div><section class=\"section-card\"><pre>");
...
const char *lm_ctrl_setup_portal_history_target(const char *uri) {
  ...
  if (strcmp(uri, "/controller-advanced") == 0) {
...
esp_err_t lm_ctrl_setup_portal_send_page(
  ...
  httpd_resp_sendstr_chunk(req,
    ...
    ".field-grid{display:grid;gap:12px;grid-template-columns:repeat(auto-fit,minmax(180px,1fr));}.preset-list{display:grid;gap:14px;margin-top:16px;}"
    ".preset-card{background:#1d140f;border:1px solid #5d4128;border-radius:18px;overflow:hidden;}.preset-card summary{list-style:none;cursor:pointer;padding:16px 18px;display:flex;justify-content:space-between;gap:16px;align-items:flex-start;}"
  ...
  send_nav(req);
  render_overview_section(req, view);
  render_controller_section(req, view);
  render_network_section(req, view);
  render_cloud_section(req, view);
